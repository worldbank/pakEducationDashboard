...
app_server <- function(input, output,session) {
  ...
  observe({
    ...
    if(query == "tab=map"){
      shiny::updateTabsetPanel(session, inputId = "data_depot", selected = "map")
    }
  })
  ...
  selection_vars_map <- callModule(mod_map_select_server, "map_select_ui_1")
  callModule(mod_map_visuals_server, "map_visuals_ui_1", selection_vars = selection_vars_map)
}
